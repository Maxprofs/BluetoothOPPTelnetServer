...
public class BluetoothOPPService extends Service
{
	...
	public void onCreate()
	{
		...
		watchdog = new Thread(new Runnable(){
				...
				public void run()
				{
					while (true)
					{
						try
						{
							...
							minutesLeft = 1;
							//Intent intent = RestartSelf();
							Log.v(TAG,"WAIT SYNC WATCHDOG");
							synchronized (mListenerThread.serversocket)
							{
								Log.d(TAG,"IN sync block WatchDog");
								try
								{
									mListenerThread.serversocket.close();
									mListenerThread.serversocket = null;
									mListenerThread.getServerSocket();
								}
								catch (IOException e)
								{
									Log.e(TAG, "err", e);
						...
						{
							Log.e(TAG, "interrupted watchdog", e);
							break;
						}
					}
					// TODO: Implement this method
				...
				private Intent RestartSelf()
				{
					stopSelf();
					Intent intent=new Intent(BluetoothOPPService.this, BluetoothOPPService.class);
	...
	class ListenerThread extends Thread
	{
		...
		private boolean StartServer()
		{
			...
			//	getServerSocket();
			Log.v(TAG,"GET SYNC STAERDRVRTG");
			//synchronized (serversocket)
		//	{
				Log.v(TAG,"IN SYNC STARTSERVER");
				if (serversocket != null)
				{
					fail = true;
		...
		private synchronized void getServerSocket()
		{
			boolean fail = true;
			while (fail)
			{
				try
				{
					...
					serversocket = new BluetoothAdapterProxy(BluetoothAdapter.getDefaultAdapter()).listenUsingRfcommOn(BluetoothAdapterProxy.CHANNEL_OPP);
					if (serversocket != null)
						fail = false;
				}
				catch (Exception e)
			...
			Log.v(TAG, "getServerSocket Successful");
			//return serversocket;
		}
		...
		private void StopOriginalListener()
		{
			//	String sClassName = "android.bluetooth.Bluetooth.BluetoothOppService";
			//BluetoothManager p= (BluetoothManager) getSystemService(BluetoothOPPService.BLUETOOTH_SERVICE);
